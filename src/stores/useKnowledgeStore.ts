...
import { getKnowledgeCards, createKnowledgeCard, updateKnowledgeCard, deleteKnowledgeCard, createField } from '@/app/(frontend)/actions';
...
export const useKnowledgeStore = create<KnowledgeStore>((set) => ({
    ...
    addKnowledgeCard: async (knowledgeCard) => {
      ...
      try {
        ...
        if (response.success && response.data) {
          set((state) => ({ knowledgeCards: [...state.knowledgeCards, response.data], loading: false }));
        } else {
    ...
    updateKnowledgeCard: async (id, data) => {
      ...
      try {
        ...
        set((state) => ({
          knowledgeCards: state.knowledgeCards.map((knowledgeCard) => (knowledgeCard.id.toString() === id ? data as Task : knowledgeCard)),
          loading: false,
        }));
    ...
    removeKnowledgeCard: async (id) => {
      ...
      try {
        ...
        set((state) => ({
          knowledgeCards: state.knowledgeCards.filter((knowledgeCard) => knowledgeCard.id.toString() !== id),
          loading: false,
          error: null
        }));
